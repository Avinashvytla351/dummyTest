...
app.post("/", (req, res) => {
  ...
  for(var i=0;i<data.length;i++){
    ...
    console.log(option.toUpperCase() === req.body[data[i].questionNo].toUpperCase())
    if(req.body[data[i].questionNo] && option.toUpperCase() === req.body[data[i].questionNo].toUpperCase()){
      score+=1;
    }
  }
  //write
  const workbook = new Excel.Workbook();
  workbook.xlsx.readFile('./results.xlsx')
    .then(() => {
